          ...
          if(Utils.CheckOrigin(descriptorURL, generatedURL)) {
            ...
            xhr.onload = function() {
              ...
              if(command.execAction.expression) {
                ...
                let obj = JSON.parse(this.responseText);x
  ...
  readURI(this.url).then(function(data) {
    try {
      ...
      for(key in commands) {
        ...
        //     if (typeof command.params[index].type.data == 'function') {
        //       command.params[index].type.data = command
        //     }
        //   }
        // }
      ...
      events.emit("commands-exported", JSON.stringify(commands));
      events.emit("commands-mod-exported", JSON.stringify(commandMods));
...
function executeCommand(name, args) {
  ...
  console.log("inside stub", commandMods);
