...
function refreshLookupData() {
  ...
  commandMods.forEach(function (command) {
    if (command.lookupData) {
      pnhLookup.push({name: command.name, lookupData: command.lookupData()}); 
    }
  });
  return pnhLookup;
}
...
function lookup() {
  /* Function to fetch the lookup data for 
  the respective command */
  return function() {
    // pnhLookupData.forEach(function (object) {
    //   // if (object.name === )
    // });
    return pnhLookupData[0].lookupData;
 }
} 
  ...
  if (command.params && command.params.length > 0) {
    for (var index = 0; index < command.params.length; index++) {
      if (command.params[index].type.name === 'selection') {
        ...
        command.params[index].type.data += "";
      }
    }
  }
});
...
exports.commandMods = commandMods;
