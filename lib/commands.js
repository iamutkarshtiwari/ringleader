...
function lookup() {
  ...
  return function () {
    name = document.getElementsByClassName("gcli-in-input")[0].value.trim();
    console.log(pnhLookupData[name].lookupData);
    return pnhLookupData[name].lookupData;
 }
}
  ...
  if (command.params && command.params.length > 0) {
    for (var index = 0; index < command.params.length; index++) {
      if (command.params[index].type.name === 'selection') {
        command.params[index].type.data = lookup();
