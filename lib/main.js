...
var button = buttons.ActionButton({
  ...
  onClick: function(state) {
    ...
    tabs.on('pageshow', function(tab) {
      ...
      // Get the list of tool commands exec()
      ...
      worker.port.on("execute", function(data) {
        ...
        if (callbackData != "") {
          console.log("PnH callback data ", callbackData);
      ...
      events.on("exec-callback-data", function(execData) {
        callbackData = JSON.parse(execData.data)[0];
        worker.port.emit("callback", {callbackData});
      });
