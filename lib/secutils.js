...
Utils.getTabFromChannel = function (aChannel) {
    ...
    if (!browser && aChannel.notificationCallbacks) {
        let loadCtx = aChannel.notificationCallbacks.QueryInterface(Ci.nsIInterfaceRequestor)
                  .getInterface(Ci.nsILoadContext);
        browser = loadCtx.topFrameElement;
    }

    console.log("\nPanel id :", (tabs_tabs.getTabForBrowser(browser)).linkedPanel);
    (browser) ? (tabs_tabs.getTabForBrowser(browser)) : null;
};
