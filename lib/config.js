...
var setup = function() {
  ...
  function handleSetup(event){
    ...
    /* TODO: Find an alternative to test if user input by mouse click or keyboard. */
  ...
  function recover(){
    console.log("current configs: ", configManager.currentConfigs());
  ...
  var doSetup = function () {
    ...
    return function () {
      ...
      if (!succeeded) {
        let mainWindow = wMediator.getMostRecentWindow("navigator:browser");
        mainWindow.gBrowser
            .addEventListener(EVENT_CONFIGURE, handleSetup, true, true);
  ...
  panel.port.on("select", function(name){
    ...
    try {
      if ('none' != name) {
        ...
        console.log("Config " + name + " applied!");
      }
    } catch (e) {
